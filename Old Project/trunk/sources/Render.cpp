...
void Render::setCountdown(int v)
{
    c_countdown = v;
}
...
void Render::draw() {
   ...
   // glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);
    ...
    if (c_countdown > 0) {
        glColor4f(1.0, 0.7, 0.7, 0.7);
        char buff[25];
        sprintf(buff, "%d", c_countdown / 60 + 1);
        int l = glutBitmapLength(GLUT_BITMAP_TIMES_ROMAN_24, (unsigned char*) buff);
        glRasterPos2f( (-float(l)  / resX), -((150) / resY - 0.5f) * 2);
